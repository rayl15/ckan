...
ALTER DEFAULT PRIVILEGES FOR USER ${CKAN_DATASTORE_RW_DB_USER} IN SCHEMA public GRANT SELECT ON TABLES TO ${CKAN_DATASTORE_RO_DB_USER};
" > toRun2.sql

echo "
ALTER TABLE geometry_columns OWNER TO ${CKAN_DATASTORE_RW_DB_USER};
ALTER TABLE spatial_ref_sys OWNER TO ${CKAN_DATASTORE_RW_DB_USER};
" > toRun3.sql

psql -f toRun0.sql
psql -f toRun1.sql -d ${CKAN_DB_NAME}
psql -f toRun2.sql -d ${CKAN_DB_NAME}
psql -f toRun3.sql -d ${CKAN_DB_NAME}
